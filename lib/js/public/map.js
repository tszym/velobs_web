...
function displayMarker(response) {
	console.log("entering displayMarker");
	document.getElementById("loader").style.display = "none";
	var json = eval('(' + response.responseText + ')');
	if (json.markers != null) {
		for (var i=0; i<json.markers.length; i++) {
			...
			console.log("Building feature " + id);
...
//	console.log("feature = %o", feature);
	...
	if (!feature.cluster){
		...
		if (featureToDisplay.attributes.desc != '') {
			htmlGeneral += "<br/><b>"+T_description+" :</b><br /> "+featureToDisplay.attributes.desc.replace(/\n/g,"<br />");
		}
		if (featureToDisplay.attributes.prop != '') {
			htmlGeneral += "<br/><b>"+T_proposition+" :</b><br /> "+featureToDisplay.attributes.prop.replace(/\n/g,"<br />");
		}
...
var mapGeoExtPanel = new GeoExt.MapPanel({
	...
	tbar: new Ext.Toolbar({
		...
		items: [dateFieldModifiedSince,CancelDateFilterItem,'&',StatusPOIFieldMapItemMenu,CancelStatusFilterItem, '&',comboPrioritePOIMapItemMenu,CancelPrioriteFilterItem,'&', numberFieldNbSupport,CancelVoteFilterItem, GetObsVotesPage]
	}),
	...
	listeners:
	{
		afterLayout: function() {
			if (afterLayoutVar == 0) {
				...
				console.log("mapPanel.afterLayout");
...
function endDrag(feat) {
	...
	console.log('feat dragué');
	Ext.Ajax.request({
			waitMsg: T_pleaseWait,
			url: 'lib/php/public/getPOIinVelobsArea.php',
			params: {
				lon: longitudeField.getValue(),
				lat: latitudeField.getValue()
			},
			success: function(o) {
			var res = Ext.util.JSON.decode(o.responseText);
			...
			Ext.MessageBox.show({
				...
				console.log('Feature in velobs area = '+o);
			},
...
function createFeatureAdd(X,Y) {
	...
	if (!eastPanelFormCreatePOI.collapsed){
		...
		//check if the feature is in the velobs area
		Ext.Ajax.request({
			waitMsg: T_pleaseWait,
			url: 'lib/php/public/getPOIinVelobsArea.php',
			params: {
...
function getMarker() {
	//clearMarker();
	console.info('Dans map.js getMarker '+tabcheckchild);
	if (tabcheckchild != '' && setDefaultCenterMapDone == 1) {
		var bounds = publicMap.getExtent().transform(projection1, projection2).toGeometry().toString();
		var alreadyLoadedObservations = featuresId.join();
		document.getElementById("loader").style.display = "block";

		var uri = 'lib/php/public/getMarker.php?listType='+tabcheckchild+'&dateLastModif='+dateLastModif+'&status='+StatusPOIFieldMapItemMenu.getValue()+'&priorite='+comboPrioritePOIMapItemMenu.getValue()+'&nbSupportMinimum='+nbSupportMinimum+'&bounds=' + bounds+'&alreadyLoadedObservations='+alreadyLoadedObservations ;
		OpenLayers.loadURL(uri, '', this, displayMarker);
		var uriCount = 'lib/php/public/getMarker.php?listType='+tabcheckchild+'&dateLastModif='+dateLastModif+'&status='+StatusPOIFieldMapItemMenu.getValue()+'&priorite='+comboPrioritePOIMapItemMenu.getValue()+'&nbSupportMinimum='+nbSupportMinimum+'&getCount=1' ;
