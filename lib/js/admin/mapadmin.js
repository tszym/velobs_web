...
var projection1 = new OpenLayers.Projection("EPSG:900913");
var projection2 = new OpenLayers.Projection("EPSG:4326");
var optionsAdminMap = {
		controls: [
...
var features = [];
var featuresId = [];
function displayNumberOfTotalRecords(response){
	console.log("entering displayNumberOfTotalRecords");
	document.getElementById("loader").style.display = "none";
	var json = eval('(' + response.responseText + ')');
	showTooltipNumberOfObservations(json.total_number_of_observations);
}
function displayMarker(response) {
	console.info('Dans mapadmin.js displayMarker');
	document.getElementById("loader").style.display = "none";
	var features = [];
	var json = eval('(' + response.responseText + ')');
	if (json.markers != null) {
		for (var i=0; i<json.markers.length; i++) {
			...
			if (!featuresId.includes(id)){
				featuresId.push(id);
			}else{
				//console.log("The feature " + id + " is still loaded");
					continue;
			}
			console.log("mapadmin.js displayMarker Building feature " + id);
			...
			if (json.markers[i].lastdatemodif_poi == null || json.markers[i].lastdatemodif_poi == '0000-00-00'){
				json.markers[i].lastdatemodif_poi = json.markers[i].datecreation_poi;
			}
			console.log("mapadmin.js displayMarker datecreation_poi " + json.markers[i].datecreation_poi);
			console.log("mapadmin.js displayMarker json.markers[i].lastdatemodif_poi " + json.markers[i].lastdatemodif_poi);
			var comments = '';
...
var tooltip = null; 
 ...
 vectorsAdminMap.events.on({
		...
		featureselected: function(e) {
			console.log(" vectorsAdminMap.events.on Click on an observation");
...
function createPopup(feature) {
	console.log("mapadmin.js createPopup entrée");
...
//	console.log("feature = %o", feature);
	...
	if (!feature.cluster){
		...
		console.log("mapadmin.js createPopup not a cluster");
		console.log("idPoiComment = " + idPoiComment);
		var dateLastModifWho = featureToDisplay.attributes.lastmodif_user_poi;
        if(dateLastModifWho == null){
        	dateLastModifWho = '?';
        }
		title = featureToDisplay.attributes.id + ", " +featureToDisplay.attributes.lib_subcategory + ', '+ T_CreationDate + ' : ' + featureToDisplay.attributes.datecreation_poi + ', '+T_lastModificationDate + ' : '  +  featureToDisplay.attributes.lastDateModif +', par ' + dateLastModifWho; 
		...
		DateCreationPOIField.setValue(featureToDisplay.attributes.datecreation_poi);
		...
		console.log("Date "+DateCreationPOIField.getValue()); 
		...
		if ((featureToDisplay.attributes.photo != null) && (featureToDisplay.attributes.photo != '')) {
			...
			PhotoPOIField.setValue(featureToDisplay.attributes.photo);
...
function getCheckChild(tab) {
	...
	if (previousTreeNodesChecked!=currentTreeNodesChecked && treeLoaderFinished == 1){
		...
		console.debug("in getCheckChild, before clearMarker and getMarker" );
		clearMarker();
		getMarker();
	}
...
function setDefaultCenterMap(response) {
	...
	if(long != null && long != 0){
		...
		console.debug("in setDefaultCenterMap, before clearMarker and getMarker" );
		getMarker();
